...
public class SaveSystem : Singleton<SaveSystem>
{
    ...
    public void LoadStickers(Dictionary<string, Sticker> stickersRef)
    {
        if (File.Exists(Application.persistentDataPath + "/playerInfo.dat"))
        {
            ...
            for (int i = 0; i < data.stickersSave.Count; i++)
            {
                ...
                {
                    data.stickersSave.Add(stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].Name, false);
                }
                else if(data.stickersSave[keyList[i]] == true)
                {
                    stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].loaded = true;
                    stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].Unlocked = data.stickersSave[keyList[i]];
                }
            }
    ...
    public void ClearStickers(Dictionary<string, Sticker> stickersRef)
    {
        ...
        List<string> keyList = new List<string>(stickersRef.Keys);
        for (int i = 0; i < stickerManRef.GetComponent<StickerManager>().stickersDic.Count; i++)
        {
            stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].loaded = false;
            stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].Unlocked = false;
        }
        ...
        for (int i = 0; i < stickerManRef.GetComponent<StickerManager>().stickersDic.Count; i++)
        {
            SaveStickers(stickerManRef.GetComponent<StickerManager>().stickersDic[keyList[i]].Name, false);
        }
    }
